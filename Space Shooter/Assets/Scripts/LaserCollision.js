...
//Main Collision Trigger
	...
	if (gameObject.tag == "laser" && col.gameObject.tag == "enemy") {
		...
		//Kill enemy
		col.gameObject.GetComponent(Enemy1Collision).Kill();
		
		//Just before destroying the laser, decrement the number of live player lasers so player firing works properly
		player.GetComponent(FireLaser).DecrementNumLiveLasers();
		Destroy(gameObject);
	}
	...
	if (col.gameObject.tag == "boundary") {
		...
		if (gameObject.tag == "enemyLaser") {
			//Just before destroying the laser, decrement the number of live lasers so enemy player firing works properly
			enemy1.GetComponent(Enemy1Fire).DecrementNumLiveLasers();
			Destroy(gameObject);
		}
